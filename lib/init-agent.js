...
// ---------------------------------------------------------------------------
// Write role definition to project AGENTS.md for session persistence
// ---------------------------------------------------------------------------
function writeRoleToAgentsMd(roleName, role) {
  const agentsMdPath = path.resolve(process.cwd(), 'AGENTS.md');
  if (!fs.existsSync(agentsMdPath)) {
    log.info('No AGENTS.md found in project root. Role will not persist across sessions.');
    return;
  }

  let content = fs.readFileSync(agentsMdPath, 'utf8');
  content = content.replace(/\n*<!-- ACTIVE ROLE:.*?-->[\s\S]*?<!-- \/ACTIVE ROLE -->\n*/g, '');

  const roleSection = `
<!-- ACTIVE ROLE: ${roleName} -->
## Active Role: ${role.title || roleName}

${formatRoleAsPrompt(role)}

<!-- /ACTIVE ROLE -->`;

  content = content.trimEnd() + roleSection;
  fs.writeFileSync(agentsMdPath, content, 'utf8');
  log.success(`Role '${roleName}' written to AGENTS.md — will persist across session turns.`);
  log.info('Start a new session for the role to take effect.');
}
...
function main(argv) {
  ...
  switch (command) {
    ...
    case '-r': {
      ...
      // Persist role definition to AGENTS.md so it survives across session turns
      writeRoleToAgentsMd(name, role);
      break;
    }
