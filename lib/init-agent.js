...
${colors.green}Commands:${colors.reset}
  ...
  init-agent --show [name]                Show current session's role definition (or role YAML with name)
...
${prompt}`);
    ...
    case '--show': {
      const name = argv[1];
      if (!name) {
        // Show current session role (model sees this and identifies itself)
        console.log(`<role-status>The "show" command was called. Report which role you are currently
following and describe your identity, capabilities, and behavior rules.

Output format:
- Role name
- Brief description
- Key capabilities relevant to the current task
- Key behavioral rules</role-status>`);
        break;
      }
